            ...
            <div className='row header'>
                <div className='col'>
                    ...
                    <nav className="navbar fixed navbar-dark overviewNavbar">
                        <a className="navbar-brand mx-auto"><h2>total: <br /> {totalTime}</h2></a>
                        <a className="navbar-brand mx-auto"><h2>today: <br /> {todaysTotalTime}</h2></a>
                    </nav>
                </div>
            </div>
                ...
                <div className='tasksList'>
                    {tasks.map((elem, idx) => {
                        return <div className='task' key={idx}>
                            ...
                            <span className="handleTimer" id={`startBtn${elem.id}`}><i className="fa fa-play" aria-hidden="true" onClick={() => startTimer(elem.id)}></i></span>
                            <span className="handleTimer" id={`pauseBtn${elem.id}`} style={{ display: 'none', color: 'red' }}><i className="fa fa-stop-circle" aria-hidden="true" onClick={() => pause(elem.id)}></i></span>
                        </div>
                    })}
                </div>
            </div>
